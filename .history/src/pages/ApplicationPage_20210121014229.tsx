...
const ApplicationPage: React.FC<Props> = () => {
  ...
  React.useEffect(() => {
    if (search === '' || search == null) return setApplicationSearch(containers)
    if (search) return setContainerSearch(containers.filter((container) => container.containerId.toLocaleLowerCase().includes(search.toLocaleLowerCase())));
  }, [containers, search])
            ...
            <TableBody>
              {usersSearch.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((user) => (
                <TableRow key={user.id}>
                  ...
                  <TableCell align='left'>{DateParser(user.timestampCreated, false)}</TableCell>
