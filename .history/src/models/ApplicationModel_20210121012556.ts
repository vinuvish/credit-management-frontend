...
export class ApplicationMode implements IApplicationModel {
  id: Number;
  firstName: string;
  lastName: string;
  ssn: string;
  ...
  static fromFirestore(doc: firebase.firestore.DocumentSnapshot<firebase.firestore.DocumentData>): ApplicationMode | null {
    const data = doc.data();
    if (!data) return null;
